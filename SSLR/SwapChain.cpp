	...
	const DXGI_SURFACE_DESC* backBufferSurfaceDesc, void* pUserContext)
{
	...
	scene_state.vFrustumParams = SimpleMath::Vector4(backBufferSurfaceDesc->Width-1, backBufferSurfaceDesc->Height-1, scene_state.mProjection._22, ratio);

	scene_state.vFrustumNearFar = SimpleMath::Vector4(np, fp, 0, 0);

	scene_state.g_FocalLen.x = 1.0f / tanf(fov * 0.5f) / ratio,
	scene_state.g_FocalLen.y = 1.0f / tanf(fov * 0.5f);
